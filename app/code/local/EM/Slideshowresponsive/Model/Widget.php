<?php
class EM_Slideshowresponsive_Model_Widget extends Mage_Widget_Model_Widget
{
	/**
    ...
    {
		if(!in_array($type,array('slideshowresponsive/create')))
			return parent::getWidgetDeclaration($type, $params, $asIs);
        $directive = '{{widget type="' . $type . '"';
		$helper = Mage::helper('slideshowresponsive');
        foreach ($params as $name => $value) {
            // Retrieve default option value if pre-configured
			if($type == 'slideshowresponsive/create'){
